...
It is then straightforward\viktor{I should check that I'm not sweeping anything under the rug by ignoring homotopies in \((\mathbf M/X)_{\leq 0}\). So far I can only prove that homotopic implies isomorphic for cofibrant and fibrant objects.}{} to check that this gives group objects \(\pi_{n}(X)\) in \(\disc(\mathbf M/X)\), which are abelian for \(n\geq 2\).
