...
We will now construct explicit \emph{truncation functors} for the
...
\begin{proposition}\label{prop:cosk-is-truncated}
  If \(X\in\sset\), then  \(\cosk_k(X)\) is \(k\)--truncated.
\end{proposition}
\begin{proof}
  \viktor{Write proof, induction over \(k\)?}
\end{proof}

\viktor{Change notation of \(\sset_{\leq k}\) so as not to conflict
  with general notation \(\mathbf M_{\leq k}\),}

\begin{proposition}
  \(\RHom(\cosk_k(X), Y) \simeq \RHom(X, Y)\) whenever \(Y\) is
  \(k\)--truncated.
\end{proposition}
