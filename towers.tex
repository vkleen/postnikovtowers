...
\[
  ...
  \end{tikzcd}
\]
wich induces commutative diagrams
\[
  \begin{tikzcd}[column sep=1em]
...
\begin{lemma}\label{lem:skeletal-simp-sets}
  ...
  \item The functors \(\hom(\_,X)\) and \(\hom(\_, \cosk_nX)\) agree
    ...
    simplicial sets, that is, composition with \(\eta\) is a natural
    isomorphism
    \[
      \eta_{*}\colon\hom(K,X)\to\hom(K,\cosk_nX)
    \]
    for \(n\)--skeletal \(K\). Furthermore, this isomorphism preserves
    pointed maps.
  \item If \(L\) is an \((n-1)\)--skeletal simplicial set and
    \(\alpha,\beta\colon L\to X\) are homotopic after composing with
    \(\eta\colon X\to\cosk_n X\),
    \ie~\(\eta\circ\alpha\simeq\eta\circ\beta\), then \(\alpha\simeq\beta\).
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  \item Since being \(n\)--skeletal is preserved under colimits and
    ...
    only consider the case \(K = \Delta^i\). \viktor{triangulate \(\Delta^i\times\Delta^1\)}
  \item Because \(K\) is assumed to be \(n\)--skeletal, the counit
    \(\varepsilon\colon \sk_nK \to K\) is an isomorphism.
  \item blub.\qedhere
  \end{enumerate}
\viktor{write proof}
\end{proof}
...
\begin{corollary}\label{cor:cosk-is-truncation}
  ...
  \(\id\to \cosk_{n}\) is an \Ths{(n-1)} truncation functor.\qed
\end{corollary}

\viktor{Prove classical homotopy group statements: relative truncation
  functors; long exact sequence; basepoint free definition}
...
\emph{\Th{n} truncation functor} for
