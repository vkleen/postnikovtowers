...
\begin{definition}[{cf.~\cite[Section~7]{gepner-kock}}]
  ...
  has a right adjoint \(\Pi_{f}\).

  A \emph{locally cartesian closed model category \(\mathbf M\)} is a
  locally cartesian closed category equipped with a model structure
  such that all adjunctions \(f^{*}\dashv \Pi_{f}\) are Quillen
  adjunctions for the slice model structures.
\end{definition}

\viktor{add abbreviation for simplicial combinatorial locally
  cartesian closed model categories.}
