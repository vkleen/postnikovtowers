...
\begin{definition}\label{defn:connect_simp}
  A map \(f\colon X\to Y\) of simplicial sets is called
  \emph{\(n\)--connected} or an \emph{\(n\)--equivalence} if for every
  \(n\)--truncated simplicial set \(Z\) the induced morphism
\[
f^{*}\colon \RHom(Y, Z) \to \RHom(X, Z)
\]
...
\begin{lemma}\label{lem:cosk_sk_composition}
  ...
  \iota^{*}\iota_{!}\) and consequently induces a natural isomorphism
...
\begin{proposition}\label{prop:cosk-is-truncated}
  If \(X\in\sset\) is a Kan complex, then  \(\cosk_n(X)\) is
  \(n\)--truncated Kan complex.
\end{proposition}
\begin{proof}
  \viktor{Write proof, induction over \(n\)?}
\end{proof}
...
\begin{proposition}
  ...
  \(\RHom(\cosk_n(X), Y) \simeq \RHom(X, Y)\) whenever \(Y\) is
  \(n\)--truncated.
\end{proposition}
\begin{proof}
  \viktor{Write proof}
...
\begin{corollary}
  ...
  \(\id\to \cosk_{n}\) is an \Th{n} truncation functor.
\end{corollary}
...
\viktor{Prove that \(\cosk_{n}\) are truncation functors. Prove
  classical homotopy group statements: truncated implies homotopy
  groups vanish in some degrees; relative truncation functors; long
  exact sequence; basepoint free definition}
